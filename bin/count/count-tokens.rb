...
def process_replacement(filename, v1, v2)
  ...
  puts "#{s1.split(/\b/).length} #{s2.split(/\b/).length}"
end
...
def process_file(filename)
  begin
    lines = File.readlines(filename).map{|x| x.encode(Encoding.find('ASCII'),$encoding_options)}
    lines.each do |l|
      l.chomp!
      case l
      when /^ *$/
...
def is_small(l, project, onechange)
  ...
  lines = File.readlines(l).map{|x| x.encode(Encoding.find('ASCII'),$encoding_options)}
  count = 0
  lines.each do |s|
    case s
    when /<@</
